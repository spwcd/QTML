...
dum = '/Users/junenyjune/Documents/1_Research/2_QTML/0_QTML_Progs/ProgramQTML/'
path = dum
...
numTrain = 30;               #trainingSize in Days
increaseSize = 10;            #how much you want to increase trainingSize per round
maxtrain  = 50;              #How many round in main Loop
...
df = pd.read_csv("Data/CPALL.csv", index_col=0, parse_dates=True)
#df = df[:80]
...
for numTPR in range(numTrain, maxtrain+increaseSize, increaseSize):
    ...
    for LoopPred in range(start_test,sizeOfX):
        ...
        X_train = X[count:LoopPred,:]
        y_train = y[count:LoopPred]
        y_train_label = y_label[count:LoopPred]
        X_test = X[LoopPred,:]
