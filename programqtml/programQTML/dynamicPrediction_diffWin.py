...
df = pd.read_csv("Data/TMB.csv", index_col=0, parse_dates=True)
#df = df[90:]   #for CPN only because it was partitioned
...
#wSize = [260, 470, 120, 120, 120, 330, 330, 120, 470, 330];  #best win for all data
wSize = [470, 260, 120, 120, 470, 330, 190, 120, 470, 400];  #best wind for only real data
#wSize = [26, 47, 12, 12, 12, 33, 33, 12, 48, 33];
lnWsize = len(wSize);
maxWsize = max(wSize);
 
#start_test = maxtrain +1;
start_test = maxWsize + 1;
sizeOfX = len(X)        
testSize = len(X) - start_test
y_test = y[start_test:] 

# create all parameters for each predictor
for x in range(1, numPredictors+1):
    ...
    #filename = str(path) +"Result/P"+ str(x) +"/"        # create folder
...
for numTest in range(start_test, sizeOfX):

    for numPred in range(1, numPredictors+1):
        trainSize = wSize[numPred-1];
        
        
        X_train = X[numTest - trainSize:numTest,:]
        y_train = y[numTest - trainSize:numTest]
    
        y_train_label = y_label[numTest - trainSize:numTest]
        X_test = X[numTest,:]
        #print("P" + str(numPred) + "train" + str(numTest - trainSize) + "-" + str(numTest))
        #print("P" + str(numPred) + "test" + str(numTest))
        
        if numPred == 4 or numPred == 5 or numPred == 9 or numPred == 10:
            globals()['P%s_y_pred' % numPred][count] = globals()['P%s_modelfiting' % numPred](X_train, y_train_label, X_test)      # predicted y 
        else :
            globals()['P%s_y_pred' % numPred][count] = globals()['P%s_modelfiting' % numPred](X_train, y_train, X_test)            # predicted y 
                
        globals()['P%s_result' % numPred][count,0] = y_test[count]                                                              # the actual y  
            
        if count == 0:
            globals()['P%s_result' % numPred][count,1] = signalCal1(globals() ['P%s_y_pred' % numPred][count])                  # Signal
        else :     
            globals()['P%s_result' % numPred][count,1] = signalCal2(globals() ['P%s_y_pred' % numPred][count], globals() ['P%s_result' % numPred][count-1,1])                      # signal from predicted y
            
        globals()['P%s_result' % numPred][count,2] = globals() ['P%s_result' % numPred][count,0] * globals() ['P%s_result' % numPred][count,1] # daily profit
...
for x in range(1, numPredictors+1):
    globals()['P%s_eva' % x] =  modelEvalute(y_test[:],  globals()['P%s_y_pred' % x][:], globals()['P%s_result' % x][:,2],0) 
    globals()['P%s_eva_df' % x]  = pd.DataFrame(list(globals()['P%s_eva' % x]))
    print(globals()['P%s_eva' % x])

#model_evluation = modelEvalute(y_test[border:],model_result[border:,0],model_result[border:,3],1)    
    
model_evluation = modelEvalute(y_test[:],model_result[:,0],model_result[:,3],1)
...
results.to_csv(str(path) +'Result/d.csv')
    
    # save detail of all results
for saveR in range(1, numPredictors+1):
    pd.DataFrame(globals()['P%s_result' % saveR]).to_csv(str(path) +'Result/P' + str(saveR) +'.csv')
     
pd.DataFrame(model_result).to_csv(str(path) +'Result/model.csv')
...
plt.ylabel('Profit')
fig.savefig(str(path) +"Result/profit.png", dpi=fig.dpi)    
'''

fig, ax = plt.subplots()
ax.plot(np.cumsum(P1_result[:,2]), color= 'black', label='P1')
ax.plot(np.cumsum(P2_result[:,2]), color = 'red', label='P2')
ax.plot(np.cumsum(P3_result[:,2]), color = 'blue', label='P3')
ax.plot(np.cumsum(P4_result[:,2]), color = 'green',label='P4')
ax.plot(np.cumsum(P5_result[:,2]), color = 'purple',label='P5')
ax.plot(np.cumsum(P6_result[:,2]), color = 'pink',label='P6')
ax.plot(np.cumsum(P7_result[:,2]), color = 'darkgray',label='P7')
ax.plot(np.cumsum(P8_result[:,2]), color = 'gold',label='P8')
ax.plot(np.cumsum(P9_result[:,2]), color = 'blueviolet',label='P9')
ax.plot(np.cumsum(P10_result[:,2]), color = 'lightskyblue',label='P10')
ax.plot(np.cumsum(model_result[:,3]), 'k--', color = 'yellow', label='Our model', linewidth = 3.0)
plt.xlabel('Trading day')
plt.ylabel('Profit')
fig.savefig(str(path) +"Result/profit.png", dpi=fig.dpi) 
